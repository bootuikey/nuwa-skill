...
```
[runtime-skills-dir]/[person-name]-perspective/
├── SKILL_DRAFT.md                    # Phase 1.5通过前只能写草稿，禁止写最终SKILL.md
├── SKILL.md                          # 最终产物（仅在Phase 4通过后创建）
...
**完成检查**（自动执行）：
- [ ] 目录已创建
- [ ] runtime skills目录已识别：Codex=`~/.codex/skills/`，Claude Code=`~/.claude/skills/`，Cursor=`~/.cursor/skills/`
- [ ] 已将女娲自带的 `scripts/validate_research_structure.py`、`scripts/quality_check.py`、`scripts/merge_research.py`、`scripts/srt_to_transcript.py`、`scripts/download_subtitles.sh` 复制到新skill的 `scripts/` 目录。复制失败时可从女娲安装目录调用，但不得跳过校验。
- [ ] 如果是中国人物：信息源策略切换为B站原始视频/YouTube公开视频/小宇宙播客/权威中文媒体优先（知乎和微信公众号始终排除，见信息源黑名单）
- [ ] 如果是跨语境人物（海外华人、全球投资人、移民背景学者/企业家）：同时使用中文源和英文源，YouTube/Podcast/英文访谈不可省略
- [ ] 如果是更新模式：已读取现有SKILL.md，标注哪些信息需要刷新
- [ ] 如果用户提供了本地语料：将素材复制/移动到 `sources/` 对应子目录，标记为**本地语料模式**

**关键规则**：
- 每个subagent必须把调研结果写入对应的md文件。不存文件的调研等于没做。
- `references/research/` 下必须存在 `01-writings.md`、`02-conversations.md`、`03-expression-dna.md`、`04-external-views.md`、`05-decisions.md`、`06-timeline.md` 六个文件。缺任何一个，禁止进入Phase 2。
- 禁止用 `README.md`、总摘要、source map 或最终 `SKILL.md` 代替六个research文件。
- 在Phase 1.5通过前，只允许写 `SKILL_DRAFT.md` 或调研文件；禁止创建最终 `SKILL.md`。如果误创建了 `SKILL.md`，必须改名为 `SKILL_DRAFT.md`，等校验通过后再恢复。
...
| 2 对话 | 播客、YouTube/B站长视频、AMA、深度采访、演讲Q&A | 被追问时的回答方式、即兴类比、改变立场的瞬间、拒绝回答的问题、口头表达DNA | `02-conversations.md` |
...
- 发现矛盾时保留矛盾，不要和稀泥
- 每个文件必须包含「来源清单」和「信息不足」两个小节；没有不足也要写「未发现明显不足」
- 如果某个Agent无法完成指定文件，不要生成最终SKILL.md；先向用户报告缺失维度和下一步补救
...
Agent 2 不允许只读二手文章。它必须优先寻找此人的长对话、公开视频、公开演讲和问答原文，尤其是YouTube/B站/Podcast transcript。

```
你的任务：调研[人名]的长对话、公开视频、演讲Q&A和即兴表达。

搜索方向：
- YouTube: "[人名] interview"、"[英文名] lecture"、"[英文名] value investing"、"[英文名] Q&A" 等
- B站/播客/高校公开视频/会议录像
- 已整理的transcript、字幕、演讲全文
- 被追问时如何回答、如何承认不知道、如何使用类比、如何拒绝问题

视频处理硬要求：
- 找到YouTube URL后，必须尝试使用 `scripts/download_subtitles.sh` 下载字幕
- 下载到SRT/VTT后，必须使用 `scripts/srt_to_transcript.py` 清洗成txt
- transcript必须保存到 `references/sources/transcripts/`
- 如果字幕不可用，必须在 `02-conversations.md` 写明尝试过的URL、失败原因，并寻找文字稿替代
- 公开视频丰富的人物，至少收集3-5个长视频/长访谈；不足时必须暂停并提示用户质量风险
- `02-conversations.md` 必须包含一个二级标题：`## 公开视频覆盖检查`

输出要求：
- 写入 [skill目录]/references/research/02-conversations.md
- 每个视频/访谈记录标题、日期、URL、语言、是否有transcript、可信度
- 提取「即兴思考模式」「表达节奏」「被追问时的反应」「不确定性表达」
- 最后写「公开视频覆盖检查」：YouTube/B站/Podcast各收集了多少，哪些明显缺失
```

#### 工具辅助（如可用）
- 书籍：Z-Library/LibGen搜索下载 → 存入 `sources/books/`
- 视频字幕获取（Agent 2找到YouTube URL时必须尝试）：
...
- 调研结构校验（Phase 1.5必须先跑）：`python3 [skill目录]/scripts/validate_research_structure.py <skill目录>`
  - 检查六个research文件是否齐全
  - 检查 `02-conversations.md` 是否包含长访谈/视频/transcript覆盖说明
  - 缺文件或对话材料不足时直接失败，禁止进入Phase 2
...
Phase 1启动前，**主动扫描当前runtime的skills目录**（Codex=`~/.codex/skills/`，Claude Code=`~/.claude/skills/`，Cursor=`~/.cursor/skills/`），检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
中文渠道只接受权威媒体：36氪、极客公园、晚点LatePost、财新、第一财经、虎嗅、少数派、机器之心等。人物访谈类可用播客平台（小宇宙、喜马拉雅原始音频）、B站原始视频（非搬运号）和YouTube公开视频。海外华人/全球投资人必须同时搜索英文YouTube、英文播客和高校/机构公开演讲。
...
```bash
python3 [skill目录]/scripts/validate_research_structure.py [skill目录]
```

失败时立刻停止，不得进入Phase 2，不得生成最终 `SKILL.md`。必须先补齐缺失文件或向用户说明无法完成的原因。

如果校验失败：
- 不要继续写最终 `SKILL.md`
- 如果已经存在 `SKILL.md`，立即改名为 `SKILL_DRAFT.md`
- 按校验输出补齐缺失文件、公开视频覆盖检查或transcript说明
- 重新运行校验，直到PASS或明确向用户报告阻塞原因
...
**视频/对话门禁**：
- `02-conversations.md` 必须存在
- 必须列出长访谈/公开视频/演讲Q&A来源
- 对公开视频丰富的人物，必须包含YouTube/B站/Podcast/transcript覆盖检查
- 如果没有任何transcript或公开视频分析，必须回到Agent 2补充，不能靠二手文章生成表达DNA
...
#### Step 4: 输出
将完成的SKILL.md写入当前runtime skills目录下的 `[person-name]-perspective/SKILL.md`（Codex默认 `~/.codex/skills/[person-name]-perspective/SKILL.md`）。

**最终写入前的硬门禁**：
1. `python3 scripts/validate_research_structure.py [skill目录]` 必须PASS
2. Phase 2.5用户确认或自动确认必须完成
3. Phase 4质量验证必须完成
4. 只有以上三项通过，才允许把 `SKILL_DRAFT.md` 改名/写入为最终 `SKILL.md`
...
| 一手来源占比 | >50% | 主要依赖二手转述 |
| 调研包完整性 | 6个research文件齐全 | 只有README或总摘要 |
| 对话/视频覆盖 | Agent 2含长访谈/视频/transcript | 没有02-conversations.md或只用二手文章 |
...
当用户说「不像本人」「语气不像」「思维方式不像」「缺少视频/访谈材料」时：
1. 不要先重写整个Skill
2. 先检查 `references/research/02-conversations.md` 和 `03-expression-dna.md` 是否存在且足够
3. 如果缺失，优先重跑Agent 2（长对话/视频/transcript）和Agent 3（表达DNA）
4. 补齐公开视频/transcript后，再回到Phase 2重新提炼心智模型和表达DNA
5. 最后跑 `validate_research_structure.py` 和 `quality_check.py`
...
### 中国人物 vs 西方人物
- **中国人物**：B站原始视频/演讲、YouTube公开视频、小宇宙播客、权威媒体采访（36氪/晚点/财新/极客公园）、本人著作/微博。知乎和微信公众号永远排除
- **西方人物**：Twitter、YouTube、Podcast、Amazon书评
- **跨语境人物/海外华人/全球投资人**：同时走中国人物和西方人物策略。YouTube、英文Podcast、高校/机构演讲、英文访谈、中文演讲都要搜；不能只用中文媒体或只用二手投资博客。李录、段永平、张磊等默认归入此类。
